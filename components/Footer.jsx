...
export default function Footer() {
  return (
    <footer style={{ background: '#060D1A', padding: '52px 24px 32px', borderTop: `1px solid ${border}` }}>
      <div className="container">
        <div className="footer-main">
          <div>
            <Logo />
            <p style={{ fontSize: 13, color: '#475569', lineHeight: 1.75, marginTop: 16, maxWidth: 280 }}>
              DevOps engineering and enterprise networking. UK-based, security-cleared, always-on.
            </p>
          </div>
          <div className="footer-cols">
            {FOOTER_COLS.map((col) => (
              <div key={col.title}>
                <div style={{ fontFamily: 'var(--font-exo2), sans-serif', fontWeight: 700, fontSize: 12, color: textPrimary, marginBottom: 16, letterSpacing: '0.05em', textTransform: 'uppercase' }}>
                  {col.title}
                </div>
                {col.links.map((l) => (
                  <div key={l.label} style={{ marginBottom: 9 }}>
                    <a
                      ...
                      onClick={l.action === 'scroll' ? (e) => { e.preventDefault(); document.getElementById(l.target)?.scrollIntoView({ behavior: 'smooth' }) } : undefined}
                      style={{ fontSize: 13, color: '#475569', transition: 'color 0.2s', cursor: 'pointer' }}
        ...
        <div style={{ borderTop: ' 1px solid rgba(163,230,53,0.07)', paddingTop: 22, display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
